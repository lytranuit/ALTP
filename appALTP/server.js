...
socket.sockets.on( 'connection', function( client ) {
		...
		if(status == 'wait'){
			for(var i=0;i<users.length;i++){
				...
				if(user.user == data.user){
					...
					client.emit('level',{level:user.level});
					client.emit('status',{status:status});
					socket.sockets.emit('add user',{user:user.user,level:user.level});
					break;
				}
			}
	...
	client.on('leave nguoichoi',function(data){
		...
		socket.sockets.emit('add user',{user:data.user,level:data.level});
	});
	client.on('start game',function(){
		if(players.length !=0){
			status = 'load game';
			socket.sockets.emit('status',{status:status});
			count = 10;
			timer = setInterval(function() {
					count--;
					socket.sockets.emit('timer',{timer:count});
					if(count <= 0){
						clearInterval(timer);
						client.emit('begin');
					}
	...
	client.on('chon user',function(){
		...
		connection.query("select * from question where level = 0", function(err, rows){
				if(err)	{
					...
					socket.sockets.emit('question',{sttquestion:sttquestion,question:question,A:A,B:B,C:C,D:D});
					socket.sockets.in('nguoichoi').emit('nguoichoi');
				}
		 });
		 count = 0;
		 timer = setInterval(function() {
			    ...
			    if(count >= 100){
				    clearInterval(timer);
				    //client.emit('begin');
					socket.sockets.emit('player sorce',{players:players});
					var minsorce = 100;
					var minplayer = null;
					...
					if(minplayer != null){
						...
						socket.sockets.emit('player select',{player:minplayer});
					}else{
						players = [];
						status = 'wait';
						socket.sockets.emit('end game');
					}
			    }
			}, 100);
	});
	client.on('start',function(){
		...
		timer = setInterval(function(){
			...
			if(count <=0){
				...
				status = 'in game';
				socket.sockets.emit('status',{status:status});
				socket.sockets.in('master').emit('next question');
			}
		},1000);
		...
		if(status == 'chon user'){
			...
			setTimeout(function(){
				if(correct_answer == data.answer){
					socket.sockets.emit('success',{correct_answer:correct_answer});
				}else{
					socket.sockets.emit('fail',{correct_answer:correct_answer});
				}
				setTimeout(function(){
						socket.sockets.emit('clear timer');
	...
	client.on('next question',function(){
		if(sttquestion == 15){
			...
			players = [];
			status = 'wait';
			socket.sockets.emit('winner',{sorce:sorce});
		}else{
			next_question();
		}
	});
	client.on('fail',function(){
		...
		players = [];
		status = 'wait';
		socket.sockets.emit('end',{sorce:sorce});
	});
	client.on('trogiup50',function(data){
		if(status == 'in game'){
			clearInterval(timer);
			delete help["trogiup50"];
			socket.sockets.emit('help',{help:help});
			var array = ["A","B","C","D"];
			var array_random = [];
			var c = 0;
			while(c < 2){
			...
			socket.sockets.emit('trogiup50',{array_random:array_random});
			hetgio();
		}
	});
...
function next_question(){
	socket.sockets.emit('reset answer');
	...
	connection.query(select_question, function(err, rows){
				if(err)	{
					...
					socket.sockets.emit('question',{sttquestion:sttquestion,question:question,A:A,B:B,C:C,D:D});
					socket.sockets.in('nguoichoi').emit('nguoichoi');
					socket.sockets.in('nguoichoi').emit('help nguoichoi');
					socket.sockets.in('master').emit('correct',{correct_answer:correct_answer});
				}
		 });
		hetgio();
	...
	timer = setInterval(function(){
		...
		if(count <=0){
			...
			setTimeout(function(){
				...
				setTimeout(function(){
					socket.sockets.emit('clear timer');
					socket.sockets.in('nguoichoi').emit('correct answer',{correct_answer:correct_answer});
				},3000);
			},3000);
		}
	},1000);
